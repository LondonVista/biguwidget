...
extension YearCalendarView {
    ...
    func monthBlock(_ month: Int, calendar cal: Calendar) -> some View {
        ...
        fmt.dateFormat = "yyyy-MM-dd"
        let wallNow = Date()
        let todayKey = fmt.string(from: wallNow)
        ...
        let snap: UsageSnapshot? = {
            if case .ready(let s) = subStore.state { return s }
            return nil
        }()
                        ...
                        ForEach(0..<7, id: \.self) { col in
                            ...
                            if dayNum >= 1 && dayNum <= daysInMonth,
                               let date = cal.date(byAdding: .day, value: dayNum - 1, to: start) {
                                ...
                                let isReset = UsageParser.isResetWeekday(key, reset: resetAt)
    ...
    func dayCell(
        ...
        return VStack(spacing: 1.5) {
            Text("\(dayNum)")
                .font(.system(size: 10.5, weight: (isToday || isReset || hasBonus) ? .bold : .semibold, design: .rounded))
                .foregroundStyle(isReset ? resetGreen : Color.white.opacity(isToday ? 0.95 : (isProjected ? 0.75 : 0.65)))
            ...
            Text(displayPercent)
                .font(.system(size: 7.8, weight: (isToday || hasBonus || isProjected) ? .bold : .semibold, design: .rounded))
        ...
        .background(
            RoundedRectangle(cornerRadius: 3.5, style: .continuous)
                .fill(isToday ? Color.white.opacity(0.07) : Color.clear)
        )
        .overlay(
            RoundedRectangle(cornerRadius: 3.5, style: .continuous)
                .stroke(isToday ? Color.white.opacity(0.22) : Color.clear, lineWidth: 0.8)
        )
        .help(
