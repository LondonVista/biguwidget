    ...
    var body: some View {
        ...
        HStack(spacing: 1) {
            ForEach(Array(days.enumerated()), id: \.element.id) { index, day in
                ...
                let isEdge = (index == 0 || index == days.count - 1)
                let isSubEdge = (index == 1 || index == days.count - 2)
                let edgeFade: Double = isEdge ? 0.58 : (isSubEdge ? 0.85 : 1.0)
                let pillOpacity: Double = (isToday || isReset) ? 1.0 : edgeFade
...
struct WeekDayPillView: View {
    ...
    private var hasBonus: Bool {
        weekStillActive && day.accumulatedGain >= 0.5
    }

    private var gainInt: Int {
        Int(day.accumulatedGain.rounded())
    }
    ...
    private var displayPercent: String {
        ...
        return "\(Int(day.percent.rounded()))%"
    }

    private var resetGreen: Color { Color(hex: 0x32D74B) }
    private var projGreen: Color { Color(hex: 0x32D74B).opacity(0.65) }
    private var gold: Color { Color(hex: 0xFFD700) }
    ...
    @ViewBuilder
    private var topDotOrBonus: some View {
        if hasBonus {
            Text("+\(gainInt)")
    ...
    private var labelView: some View {
        let fgColor: Color = {
            if isReset { return resetGreen }
            return isToday ? Color.white.opacity(0.9) : Color.white.opacity(0.5)
        }()
        return Text(day.label)
            .font(.system(size: 8, weight: (isToday || isReset || hasBonus) ? .bold : .semibold))
            .foregroundStyle(fgColor)
            .padding(.horizontal, isReset ? 3.5 : 0)
            .padding(.vertical, isReset ? 0.5 : 0)
            .background(
                Group {
                    if isReset {
                        RoundedRectangle(cornerRadius: 3, style: .continuous)
                            .fill(resetGreen.opacity(0.20))
                    }
                }
            )
            .overlay(
                Group {
                    if isReset {
                        RoundedRectangle(cornerRadius: 3, style: .continuous)
                            .stroke(resetGreen.opacity(0.45), lineWidth: 0.7)
                    }
                }
            )
    }
    ...
    private var pillBackground: some View {
        let bgFill: Color = {
            if isToday { return Color.white.opacity(0.07) }
            return Color.clear
        }()
    ...
    private var pillBorder: some View {
        let strokeColor: Color = {
            if isToday { return Color.white.opacity(0.22) }
            return Color.clear
        }()
        return RoundedRectangle(cornerRadius: 3.5, style: .continuous)
            ...
            case .ready(let snap):
                ...
                if subStore.isCollapsed {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        if !isWeekHidden {
                            let weekDays = subStore.daysForDisplay(now: wallNow, resetsAt: snap.resetsAt, centerToday: settings.centerTodayInWeekStrip)
                            WeekStripView(
                                days: weekDays,
                                todayKey: todayKey,
